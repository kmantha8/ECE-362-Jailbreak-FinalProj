...
uint32_t score = 0;
uint32_t sound_off_time = 0;
uint16_t current_volume = 2048;
...
int main() {
    ...
    // 1. HARDWARE INIT (Do this only once!)
    adc_init();
    adc_gpio_init(41);
    adc_select_input(1);
    ...
    while (true) {
        ...
        if (sound_off_time != 0 && time_us_32() > sound_off_time) {
            pwm_set_gpio_level(20, 0); // Pin 20 is SPEAKER_PIN
            sound_off_time = 0;
        }
            ...
            if (!lose_end) {
                ...
                for (int i = 0; i < MAX_BOX_ROWS; i++)
                {
                        ...
                        )
                    {
                        ...
                        sound_off_time = time_us_32() + 30000;
                    }
